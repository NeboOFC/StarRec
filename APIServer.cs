...
namespace server
{
	internal class APIServer
	{
			...
			try
			{
				...
				if (start.Program.version == "2017")
				{
					for (; ; )
					{
						...
						if (Url == "platformlogin/v1/profiles")
						{
							...
							if (new WebClient().DownloadString("https://raw.githubusercontent.com/NeboOFC/StarRec/main/Update/banned.txt?token=GHSAT0AAAAAACUBHYZXPIQK2ARFKCUO3UEIZUG6XIQ").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
							{
				...
				else if (start.Program.version == "2018")
				{
					for (; ; )
					{
						...
						{
							if (new WebClient().DownloadString("https://raw.githubusercontent.com/NeboOFC/StarRec/main/Update/banned.txt?token=GHSAT0AAAAAACUBHYZWXC5SBQF4I6N4FOHOZUHS3DQ").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
							{
								Console.ForegroundColor = ConsoleColor.Red;
								Console.WriteLine("You are banned. Using this version of StarRec will not work, please download StarRec 0.0.2 or prior.");
								Console.ForegroundColor = ConsoleColor.DarkMagenta;
						...
						if (Url == "rooms/v1/featuredRoomGroup")
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/NeboOFC/StarRec/main/Update/dormslideshow.txt?token=GHSAT0AAAAAACUBHYZXHMY26U3GNIAETEHEZUHTAIQ");
						}
						if (Url.StartsWith("rooms/v1/hot"))
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/NeboOFC/StarRec/main/Update/hotrooms.txt?token=GHSAT0AAAAAACUBHYZWF5IFB4NBLO6EWVEGZUHTANA");
						}
						...
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/NeboOFC/StarRec/main/Update/rcslideshow.txt?token=GHSAT0AAAAAACUBHYZWVLEDFV3TM72KG5RMZUHTAYA");
						}
						Console.WriteLine("API Response: " + s);
						bytes = Encoding.UTF8.GetBytes(s);
						response.ContentLength64 = (long)bytes.Length;
		...
		public static string ChallengesV1GetCurrent = "{\"Success\":true,\"Message\":\"StarRec\"}";
